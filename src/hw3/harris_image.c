...
image structure_matrix(image im, float sigma)
{
	...
	image f_gaussian = make_gaussian_filter(sigma);
	S = convolve_image(S, f_gaussian, 1); 
...
image cornerness_response(image S)
{
	...
	for(j=0; j<R.h; ++j){
		for(i=0; i<R.w; ++i){
			...
			float ix_ix = get_pixel(S, i, j, 0 ); //1st channel is Ix^2 
			float iy_iy = get_pixel(S, i, j, 1 ); //2nd channel is Iy^2 
			float ix_iy = get_pixel(S, i, j, 2 ); //3rd channel is Ix*Iy
			float det_S = ix_ix * iy_iy - ix_iy * ix_iy; 
			float trace_S = ix_ix + iy_iy; 
			float v = det_S - 0.06*trace_S*trace_S; //det(S) - alpha * trace(S)^2, alpha = .06.
			set_pixel(R, i, j, 0, v); 
		}
	}
    return R;
...
image nms_image(image im, int w)
{
	...
	for(j=0; j<im.h; ++j){
		for(i=0; i<im.w; ++i){
			float center_v = get_pixel(im, i, j, 0); //center pixel
			for(l=-1*w; l<=w; ++l){
				for(m=-1*w; m<=w; ++m){
					float n_v = get_pixel(im, i+m, j+l, 0); //neighboring pixel
					if( n_v > center_v){ // if neighbor response greater than pixel response
    ...
    *n = count; // <- set *n equal to number of corners in image.
	...
	for(j=0; j<Rnms.h; ++j){
		for(i=0; i<Rnms.w; ++i){
			float v = get_pixel(Rnms, i, j, 0);
			if( v >= thresh ){
				int pixel_index = i + Rnms.w*j;
				d[desc_i] = describe_index(im, pixel_index);
				desc_i += 1;
			}  
		}
	}
